...
- **"Disabled for Trusted Networks"** — a subset of IPs/CIDRs can skip login. Default is all RFC1918 private networks + loopback (matches Radarr's behavior). Can be narrowed to a custom list (`192.168.0.0/24, 10.0.0.0/24`) so only specific subnets bypass auth — useful if you want your home VLAN and WireGuard tunnel trusted but not every random device on another VLAN.
