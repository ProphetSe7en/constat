...
// the main Constat UI for visual consistency.
...
<head>
<meta charset="utf-8">
<title>Constat — First-run setup</title>
...
<body>
<div class="card">
  <h1>Welcome to Constat</h1>
  <p class="sub">Authentication is now required. Create the admin account to continue — you'll use this login every time you access Constat from outside your local network.</p>
  ...
  <footer>Constat {{.Version}} · security · <a href="https://github.com/prophetse7en/constat" style="color:#4a90e2;text-decoration:none">github</a></footer>
</div>
</body>
</html>`))
...
func (h *AuthHandlers) handleAuthStatus(w http.ResponseWriter, r *http.Request) {
	...
	// and mode=none callers. All three reach admin-gated UI endpoints via the
	// middleware, so they must see the same view as authenticated users on
	// /api/auth/status — otherwise the Security panel renders with stale
	// lock-state and Save fails later (user sees "input editable" / "Save
	// refused"). Username/lock-flags/effective values leak no more to a
	// local-bypass peer than the admin-gated API-key endpoint already does.
	...
	if adminEquivalent {
		...
		// When locked, surface the effective env-var string so the UI can
		// show what's actually enforced (the app's config-file value may be
		// stale). Value is operationally sensitive; adminEquivalent gate
		// ensures only people already at admin-tier see it.
